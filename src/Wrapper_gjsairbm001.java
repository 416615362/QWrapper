 ...
 *
 */
public class Wrapper_gjsairbm001 implements QunarCrawler{
	...
	public static void main(String[] args) {
		...
		//ABZ EBJ 2014-07-14 2014-08-06
		//BRE TLS 2014-07-30 2014-08-16
		//BRU NCL 2014-7-10 2014-7-29
		searchParam.setDep("BRE");
		searchParam.setArr("TLS");
		searchParam.setDepDate("2014-07-30");
		searchParam.setRetDate("2014-08-16");
		searchParam.setTimeOut("60000");
		searchParam.setWrapperid("gjsairbm001");
		searchParam.setToken("");
		
		// 获取请求返回的html
		String html = new  Wrapper_gjsairbm001().getHtml(searchParam);
		ProcessResultInfo result = new ProcessResultInfo();
		// 拼装返回的结果
		result = new  Wrapper_gjsairbm001().process(html,searchParam);
	...
	public String getHtml(FlightSearchParam param) {
		...
		try {	
			...
			// 请求地址
			String getUrl = String.format("http://www.bmiregional.com/en/Flight/Search?interline=false&fromCityCode=%s&toCityCode=%s&departureDateString=%s&returnDateString=%s&adults=1&children=0&infants=0&roundTrip=true&useFlexDates=false&allInclusive=undefined&promocode=&fareTypes=undefined&currency=%s",
					param.getDep(),param.getArr(),param.getDepDate(),param.getRetDate(),currency);
			...
			if(get.getStatusCode() == HttpStatus.SC_MOVED_TEMPORARILY || get.getStatusCode() == HttpStatus.SC_MOVED_PERMANENTLY){
				...
				String startDateTimeOutbound = sdf.format(cal.getTime());
				// 计算去程endDate时间
				cal.clear();
				cal.setTime(sdf.parse(startDateTimeOutbound));
				cal.add(Calendar.DATE, 30);
				String endDateTimeOutbound = sdf.format(cal.getTime());
				// 计算返程程startDate时间
				cal.clear();
				Date arrDate = sdf.parse(param.getRetDate());
				cal.setTime(arrDate);
				cal.add(Calendar.DATE, -15);
				String startDateTimeInbound = sdf.format(cal.getTime());
				// 计算去程endDate时间
				cal.clear();
				cal.setTime(sdf.parse(startDateTimeInbound));
				cal.add(Calendar.DATE, 30);
				String endDateTimeInbound = sdf.format(cal.getTime());
				// json参数
				String jsonbody = "{\"interline\":false,\"fromCityCode\":\""+param.getDep()+"\",\"toCityCode\":\""+param.getArr()+"\",\"departureDateString\":\""+param.getDepDate()+"\",\"returnDateString\":\""+param.getRetDate()+"\",\"startDateStringOutbound\":\""+startDateTimeOutbound+"\",\"endDateStringOutbound\":\""+endDateTimeOutbound+"\",\"startDateStringInbound\":\""+startDateTimeInbound+"\",\"endDateStringInbound\":\""+endDateTimeInbound+"\",\"adults\":1,\"children\":0,\"infants\":0,\"roundTrip\":true,\"useFlexDates\":false,\"isOutbound\":true,\"filterMethod\":\"100\",\"promocode\":\"\",\"currency\":\""+currency+"\"}";
	...
	public ProcessResultInfo process(String html, FlightSearchParam param) {
		...
		try {
			...
			// 去程json
			String outboundHtml = StringUtils.substringBetween(html, "OutboundSegments\":", ",\"InboundSegments\"");
			if(outboundHtml.indexOf("Legs") <= -1){
				result.setRet(false);
				result.setStatus(Constants.NO_RESULT);
				return result;
			}
			// 获取对应的值
			String[] jsonStr = StringUtils.substringsBetween(outboundHtml, "Legs\":", ",\"FaresTypes\"");
			String[] jsonPriceStr = StringUtils.substringsBetween(outboundHtml, "FaresTypes\":", ",\"LowestFareSummary\"");
			for(int ii=0;ii<jsonStr.length;ii++){
				// 判断是否为空
				if (StringUtils.isEmpty(jsonStr[ii]) || StringUtils.isEmpty(jsonPriceStr[ii])) {
					result.setRet(false);
					result.setStatus(Constants.NO_RESULT);
					return result;
				}
				if(!StringUtils.isEmpty(jsonStr[ii]) && !StringUtils.isEmpty(jsonPriceStr[ii])){
					// 航班数组
					JSONArray ajson = JSON.parseArray(jsonStr[ii]);
					// 金额数组
					JSONArray ajsonPrice = JSON.parseArray(jsonPriceStr[ii]); // 价格json
					// 去程航班信息
					OneWayFlightInfo goBaseFlight = new OneWayFlightInfo();
					FlightDetail goFlightDetail = new FlightDetail();
					...
					for (int i = 0; i < ajson.size(); i++) {
						...
						/************添加FlightDetail信息****************/
						// 出发时间
						goFlightDetail.setDepdate(sdf.parse(depDate));
						// 航班号列表
						goFlightDetail.setFlightno(goFlightNoList);
						// 价格，该航班上的最低价
						goFlightDetail.setPrice(Double.parseDouble(amount));
						// 税
						goFlightDetail.setTax(tax);
						// 货币单位
						goFlightDetail.setMonetaryunit(curr);
						// 出发三字码
						goFlightDetail.setDepcity(org);
						// 到达三字码
						goFlightDetail.setArrcity(dst);
						// 抓取器id
						goFlightDetail.setWrapperid(param.getWrapperid());
						// 添加单程航班信息集合
						goBaseFlight.setDetail(goFlightDetail);
			...
			// 返程json
			String inboundHtml = StringUtils.substringBetween(html, "InboundSegments\":", ",\"Criteria\"");
			if(inboundHtml.indexOf("Legs") <= -1){
				result.setRet(false);
				result.setStatus(Constants.NO_RESULT);
				return result;
			}
			// 获取对应的值
			String[] retJsonStr = StringUtils.substringsBetween(inboundHtml, "Legs\":", ",\"FaresTypes\"");
			String[] retJsonPriceStr = StringUtils.substringsBetween(inboundHtml, "FaresTypes\":", ",\"LowestFareSummary\"");
			for(int ii=0;ii<retJsonStr.length;ii++){
				// 判断是否为空
				if (StringUtils.isEmpty(retJsonStr[ii]) || StringUtils.isEmpty(retJsonPriceStr[ii])) {
					result.setRet(false);
					result.setStatus(Constants.NO_RESULT);
					return result;
				}
				if(!StringUtils.isEmpty(retJsonStr[ii]) && !StringUtils.isEmpty(retJsonPriceStr[ii])){
					// 航班数组
					JSONArray retJson = JSON.parseArray(retJsonStr[ii]);
					// 金额数组
					JSONArray retJsonPrice = JSON.parseArray(retJsonPriceStr[ii]); // 价格json
					// 返程航班信息
					...
					for (int i = 0; i < retJson.size(); i++) {
						// 航班对象
						JSONObject jsonObject = retJson.getJSONObject(i);
						FlightSegement seg = new FlightSegement();
						// 出发机场三字码
						JSONObject orgObject = jsonObject.getJSONObject("Origin");
						String org = String.valueOf(orgObject.get("Code"));
						...
						// 金额对象
						JSONObject jsonPriceObject = retJsonPrice.getJSONObject(i);
						JSONArray faresArray = jsonPriceObject.getJSONArray("Fares");
						JSONObject faresObject = faresArray.getJSONObject(0);
						// 价格
						String amount = String.valueOf(faresObject.get("Amount"));
						...
						/************添加FlightDetail信息****************/
						// 出发时间
						retFlightDetail.setDepdate(sdf.parse(depDate));
						// 航班号列表
						retFlightDetail.setFlightno(retFlightNoList);
						// 价格，该航班上的最低价
						retFlightDetail.setPrice(Double.parseDouble(amount));
						// 税
						retFlightDetail.setTax(tax);
						// 货币单位
						retFlightDetail.setMonetaryunit(curr);
						// 出发三字码
						retFlightDetail.setDepcity(org);
						// 到达三字码
						retFlightDetail.setArrcity(dst);
						// 抓取器id
						retFlightDetail.setWrapperid(param.getWrapperid());
						// 添加单程航班信息集合
	...
	public BookingResult getBookingInfo(FlightSearchParam param) {
		...
		try {
			...
			map.put("returnDateString", param.getRetDate());
			map.put("adults", "1");
			map.put("children", "0");
			map.put("infants", "0");
			map.put("roundTrip", "true");
